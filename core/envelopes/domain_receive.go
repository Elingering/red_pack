...
func (d *goodsDomain) Receive(
	...
	err = base.Tx(func(runner *dbx.TxRunner) error {
		...
		if status == acservices.TransferStatusSuccess {
			return nil
		} else {
			return err
		}
...
func (d *goodsDomain) transfer(
	ctx context.Context,
	dto services.RedEnvelopeReceiveDTO) (status acservices.TransferStatus, err error) {
	...
	if err != nil || status != acservices.TransferStatusSuccess {
		return status, err
	}
	//从系统红包资金账户转入当前用户
	transfer = acservices.AccountTransferDTO{
...
func (d *goodsDomain) nextAmount(goods *RedEnvelopeGoods) (amount decimal.Decimal) {
	...
	} else if goods.EnvelopeType == int(services.LuckyEnvelopeType) {
